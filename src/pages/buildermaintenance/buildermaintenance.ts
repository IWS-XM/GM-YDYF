...
refresh(refreshTab): Promise<any> {
  return new Promise((resolve) => {
      ...
      // this.nativeservice.showLoading("处理中，请稍侯...");
      resolve(promise.then((v1) => {
          return this.initBaseDB.getbuilderissuelist(this.projid, 4, this.assignfilterstr, this.buildingfilterstr, this.floorfilterstr, this.duedatefilterstr, this.returntimesfilterstr, this.fixdatefilterstr, this.checkitemfilterstr, this.sortingname, refreshTab);
      }).then((val: any) => {
          console.log("refresh end");
          if (val) {
              let list: Array<any>; list = [];
