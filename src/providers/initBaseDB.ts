...
export class initBaseDB {
          ...
          for (var i = 0; i < records.length; i++) {
            ...
            tmppromise = tmppromise.then(() => {
              return tmprecords.push({
                ...
                ReassignDate: values.ReassignDate, ReassignDesc: values.ReassignDesc, ReasonbyOver: values.ReasonbyOver, FixedDesc: values.FixedDesc, ImgReturn1: values.ImgReturn1, ImgReturn2: values.ImgReturn2
              });
            }).then((v) => {
              return tmprecords;
            })
    ...
    return new Promise((resolve) => {
      ...
      resolve(promise.then((v1:number) => {
        ...
        for (var i = 0; i < buildings.length; i++) {
          console.log(JSON.stringify((buildings[i])));
        ...
        // this.nativeservice.hideLoading();
        console.log('部分更新错误:' + err);
      }))
    })
  }

  updateallbuildings(token, projid, type, batchbuildings): Promise<any> {
    return new Promise((resolve) => {
      let promise = new Promise((resolve) => {
        resolve(100);
      });
      let loadissueimg: boolean;
      resolve(promise.then((vv) => {
        return this.nativeservice.isConnecting();
      }).then((val: boolean) => {
        if (val == false) {
          throw '';//resolve(10);
        } else {
          return this.dialogs.confirm('是否下载全部房间问题图片', '', ['取消下载', '确定下载'])
          //this.initBaseDB.downloadbuildinginfo(this.token, this.projid, batchid, building.buildingid, this.type);
        }
      }).then((v1:number) => {
        let tmppromise = Promise.resolve([]);
        for (var i = 0; i < batchbuildings.length; i++) {
        ...
        console.log("全部更新完成");
        this.nativeservice.hideLoading();
        this.nativeservice.showToast('全部更新完成.');
        return v;
      }).catch(err => {
        this.nativeservice.hideLoading();
        console.log('全部更新错误:' + err);
      }))
    })
  }

  downloadbuildinginfo(token, projid, batchid, buildingid, type, loadimg = 0): Promise<any> {
    return new Promise((resolve) => {
      let promise = new Promise((resolve) => {
        resolve(100);
      });
      ...
      resolve(promise.then((v1) => {
        ...
        return this.updatebuildinginfo(token, projid, batchid, buildingid, type, loadimg);
      }).catch(err => {
  ...
  uploadbuildinginfo(token, projid, batchid, buildingid, type, loadimg = 0): Promise<any> {
    return new Promise((resolve) => {
      let promise = new Promise((resolve) => {
        resolve(100);
      });
  ...
  updatebuildinginfo(token, projid, batchid, buildingid, type, loadimg = 0): Promise<any> {
    return new Promise((resolve) => {
  ...
  updateDynamicsPack(data, projid, batchid, buildingid, type, loadissueimg: boolean): Promise<any> {
    return new Promise((resolve) => {
      ...
      resolve(promise.then((v1) => {
        ...
        sql += " union select ImgReturn1 as fn from #tmptablename# where ImgReturn1 != '' and ImgReturn1 != 'NUll' and not exists (select projid from imagetable where imagetable.projid = '#projid#' and imagetable.fn = #tmptablename#.ImgClose1) ".replace('#tmptablename#', tmptn).replace('#tmptablename#', tmptn).replace('#projid#', projid);
        sql += " union select ImgReturn2 as fn from #tmptablename# where ImgReturn2 != '' and ImgReturn2 != 'NUll' and not exists (select projid from imagetable where imagetable.projid = '#projid#' and imagetable.fn = #tmptablename#.ImgClose2) ".replace('#tmptablename#', tmptn).replace('#tmptablename#', tmptn).replace('#projid#', projid);
        if (type == 2) {
          sql += " union select ImgSign as fn from #tmptablename2# where ImgSign != '' and ImgSign != 'NUll' and projid = '#projid#' and batchid = '#batchid#' and buildingid = '#buildingid#' and not exists (select projid from imagetable where imagetable.projid = '#projid#' and imagetable.fn = #tmptablename2#.ImgSign) ".replace('#tmptablename2#', 'tmpOpenRoomDetails').replace('#tmptablename2#', 'tmpOpenRoomDetails').replace('#projid#', projid);
        } else if (type == 3) {
      ...
      resolve(promise.then((v1) => {
        ...
        for (let k = 0; k < tablename.length; k++) {
          ...
          tmppromise = tmppromise.then(() => {
            let sql: string;
            if (tn == "ProjTeam") {
              sql = "delete from ProjTeam where projid = '" + projid + "'";//id like 'builder%' and 
            } else if (tn == "Vend") {
              sql = "delete from Vend where projid = '" + projid + "'";//id like 'builder%' and 
            } else {
        ...
        sql += " union select ImgReturn1 as fn from #tmptablename# where ImgReturn1 != '' and ImgReturn1 != 'NUll' and (IssueStatus = '待派单' or IssueStatus = '待整改' or IssueStatus = '已整改') and not exists (select projid from imagetable where imagetable.projid = '#projid#' and imagetable.fn = #tmptablename#.ImgReturn1) ".replace('#tmptablename#', 'tmpPreCheckIssues').replace('#tmptablename#', 'tmpPreCheckIssues').replace('#projid#', projid);
        sql += " union select ImgReturn2 as fn from #tmptablename# where ImgReturn2 != '' and ImgReturn2 != 'NUll' and (IssueStatus = '待派单' or IssueStatus = '待整改' or IssueStatus = '已整改') and not exists (select projid from imagetable where imagetable.projid = '#projid#' and imagetable.fn = #tmptablename#.ImgReturn2) ".replace('#tmptablename#', 'tmpPreCheckIssues').replace('#tmptablename#', 'tmpPreCheckIssues').replace('#projid#', projid);
        ...
        sql += " union select ImgReturn1 as fn from #tmptablename# where ImgReturn1 != '' and ImgReturn1 != 'NUll' and (IssueStatus = '待派单' or IssueStatus = '待整改' or IssueStatus = '已整改') and not exists (select projid from imagetable where imagetable.projid = '#projid#' and imagetable.fn = #tmptablename#.ImgReturn1) ".replace('#tmptablename#', 'tmpOpenCheckIssues').replace('#tmptablename#', 'tmpOpenCheckIssues').replace('#projid#', projid);
        sql += " union select ImgReturn2 as fn from #tmptablename# where ImgReturn2 != '' and ImgReturn2 != 'NUll' and (IssueStatus = '待派单' or IssueStatus = '待整改' or IssueStatus = '已整改') and not exists (select projid from imagetable where imagetable.projid = '#projid#' and imagetable.fn = #tmptablename#.ImgReturn2) ".replace('#tmptablename#', 'tmpOpenCheckIssues').replace('#tmptablename#', 'tmpOpenCheckIssues').replace('#projid#', projid);
        ...
        sql += " union select ImgReturn1 as fn from #tmptablename# where ImgReturn1 != '' and ImgReturn1 != 'NUll' and (IssueStatus = '待派单' or IssueStatus = '待整改' or IssueStatus = '已整改') and not exists (select projid from imagetable where imagetable.projid = '#projid#' and imagetable.fn = #tmptablename#.ImgReturn1) ".replace('#tmptablename#', 'tmpFormalCheckIssues').replace('#tmptablename#', 'tmpFormalCheckIssues').replace('#projid#', projid);
        sql += " union select ImgReturn2 as fn from #tmptablename# where ImgReturn2 != '' and ImgReturn2 != 'NUll' and (IssueStatus = '待派单' or IssueStatus = '待整改' or IssueStatus = '已整改') and not exists (select projid from imagetable where imagetable.projid = '#projid#' and imagetable.fn = #tmptablename#.ImgReturn2) ".replace('#tmptablename#', 'tmpFormalCheckIssues').replace('#tmptablename#', 'tmpFormalCheckIssues').replace('#projid#', projid);
        ...
        sql += " union select ImgReturn1 as fn from #tmptablename# where ImgReturn1 != '' and ImgReturn1 != 'NUll' and (IssueStatus = '待派单' or IssueStatus = '待整改' or IssueStatus = '已整改') and not exists (select projid from imagetable where imagetable.projid = '#projid#' and imagetable.fn = #tmptablename#.ImgReturn1) ".replace('#tmptablename#', 'tmpServiceCheckIssues').replace('#tmptablename#', 'tmpServiceCheckIssues').replace('#projid#', projid);
        sql += " union select ImgReturn2 as fn from #tmptablename# where ImgReturn2 != '' and ImgReturn2 != 'NUll' and (IssueStatus = '待派单' or IssueStatus = '待整改' or IssueStatus = '已整改') and not exists (select projid from imagetable where imagetable.projid = '#projid#' and imagetable.fn = #tmptablename#.ImgReturn2) ".replace('#tmptablename#', 'tmpServiceCheckIssues').replace('#tmptablename#', 'tmpServiceCheckIssues').replace('#projid#', projid);
        ...
        console.log(data[1][0]);
        let sql = "update projversion set lastdate = '#lastdate#' where projid = '#projid#'";
        sql = sql.replace('#projid#', projid);
  ...
  uploadbuilderinfo(token, projid): Promise<any> {
    return new Promise((resolve) => {
      ...
      resolve(promise.then((v) => {
        ...
        let sql = "update projversion set needupd = 0 where projid = '#projid#'";
        sql = sql.replace('#projid#', projid);
        console.log('updbdataupd:' + sql);
        return this.db.executeSql(sql, []);
      }).then((v5) => {
        console.log('v5')
        return this.downloadbuilderdata(token, projid);
    ...
    return new Promise((resolve) => {
      ...
      resolve(promise.then(v => {
        ...
        if (v2 && v2.rows && v2.rows.length > 0) {
          ...
          let issforass: Array<string>; issforass = [];
          let issforfix: Array<string>; issforfix = [];
          let issfixed: Array<string>; issfixed = [];
          let issreturned: Array<string>; issreturned = [];
  ...
  getbuilderissueinfo(issueid, type): Promise<any> {
    return new Promise((resolve) => {
      ...
      let ret = [];
      resolve(promise.then((v1) => {
        let sql = "select iss.fixedDesc,iss.ReFormDate,iss.ImgBefore1,iss.ImgBefore2,iss.ImgBefore3,iss.ImgAfter1,iss.ImgAfter2,iss.ImgAfter3,iss.IssueStatus,iss.AppointDate,iss.LimitDate,iss.RegisterDate,iss.VendId,iss.PlusDesc,iss.APImg,iss.ImgReturn1,iss.ImgReturn2 from #tablename# iss where iss.Id = '#issueid#'";
  ...
  //Id,NameAlias,Manager,ManagerName,Phone,Projid,ResponsibleId  
  getProjTeamVendors(projid): Promise<any> {
    return new Promise((resolve) => {
      ...
      resolve(promise.then((v1) => {
        ...
        for (var i = 0; i < v2.rows.length; i++) {
          console.log(JSON.stringify(v2.rows.item(i)));
          teamvendors.push({ vendid: v2.rows.item(i).Id, name: v2.rows.item(i).NameAlias, managerphone: v2.rows.item(i).Phone, managername: v2.rows.item(i).ManagerName });
        }
        return teamvendors;
      }).catch(err => {
        this.warn('项目团队承建商加载失败:' + err);
        throw '项目团队承建商加载失败';
      }))
    })
  }
  ...
  addProjTeamMembers(projid, vendid, phone, values): Promise<any> {
    return new Promise((resolve) => {
      let promise = new Promise((resolve) => {
        resolve(100);
      });
      ...
      resolve(promise.then((v1) => {
        let sql = "select Phone from projteam where projid = '" + projid + "' and vendid = '" + vendid + "' and phone = " + phone.toString();
        console.log(sql);
        return this.db.executeSql(sql, []);
      }).then((v2: any) => {
        if (v2 && v2.rows.length > 0) {
          ...
          let sql = "insert into ProjTeam(Id,projid,userid,phone,name,vendId) values (#values#)";
    ...
    return new Promise((resolve) => {
      ...
      resolve(promise.then((v) => {
        if (v == false) {
          ...
          if (issuelist.ImgReturn1) {
            sql += " union select '#imgr1#' as fn where not exists (select projid from imagetable where imagetable.projid = '#projid#' and imagetable.fn = '#imgr1#') ".replace('#projid#', projid).replace('#imgr1#', issuelist.ImgReturn1).replace('#imgr1#', issuelist.ImgReturn1);

            if (issuelist.ImgReturn2) {
              sql += " union select '#imgr2#' as fn where not exists (select projid from imagetable where imagetable.projid = '#projid#' and imagetable.fn = '#imgr2#') ".replace('#projid#', projid).replace('#imgr2#', issuelist.ImgReturn2).replace('#imgr2#', issuelist.ImgReturn2);
            }
          }
